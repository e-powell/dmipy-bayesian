    ...
    for j in range(0, nsteps):
        ...
        for roi in range(nroi):  # FIXME: range(nroi) gives e.g. 0,1 when just 1 ROI. Better to use nroi.length?
            ...
            for roi in range(nroi):
                ...
                prior_for_rois[idx_roi, roi] = scipy.stats.multivariate_normal.pdf(parameter_vector[idx_roi,:], gibbs_mu[roi, :, j], gibbs_sigma[roi, :, :, j], allow_singular=1)
        ...
        # randomly sample roi membership using multinomial distribution
        sample = np.zeros((nvox, nroi))
        sample[idx_mask,:] = np.array([np.random.multinomial(100, prior_for_rois[x,:], size=None) for x in idx_mask])/100
