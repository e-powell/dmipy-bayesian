...
for i in snr:
    ...
    parameters_bayes_dict[i], acceptance_rate[i], parameter_convergence[i], likelihood[i], weights[i], gibbs_mu_tform[i], gibbs_sigma_tform[i], gibbs_mu_norm[i], gibbs_sigma_norm[i]        = fit_bayes_new.fit(mc_smt_mod, acq_scheme_smt, signals_sm[i], E_fit[i], mc_smt_mod_lsq_fit[i].fitted_parameters, ROImask[i], nsteps, burn_in, nupdates)
