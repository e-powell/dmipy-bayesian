...
class G1BallT2(ModelProperties, IsotropicSignalModelProperties):
    r""" The Ball model [1]_ - an isotropic Tensor with one diffusivity
    with additional T2 decay
    ...
    .. [1] Behrens et al.
           ...
           Magnetic Resonance in Medicine (2003)
    """
    _required_acquisition_parameters = ['bvalues', 'TE']
    ...
    def __init__(self, lambda_iso=None, t2=None):
        self.lambda_iso = lambda_iso
        self.t2 = t2
    ...
    def __call__(self, acquisition_scheme, **kwargs):
        ...
        E_ball = np.exp(-bvals * lambda_iso)
        E_ballT2 = np.exp(-(te - np.min(te))/t2_)*E_ball
        #E_ballT2 = np.exp(-te/t2_)*E_ball
        return E_ballT2


class G1KurtosisT2(ModelProperties, IsotropicSignalModelProperties):
    r""" The Ball model [1]_ - an isotropic Tensor with one diffusivity
    with additional T2 decay
    ...
    def __init__(self, lambda_iso=None, t2=None, kurtosis=None):
        self.lambda_iso = lambda_iso
        self.kurtosis = kurtosis
        self.t2 = t2
    ...
    def __call__(self, acquisition_scheme, **kwargs):
        ...
        E_kurtosis = np.exp(-bvals * lambda_iso + (1/6) * bvals**2 * lambda_iso**2 * kurtosis)
        E_kurtosisT2 = np.exp(-(te - np.min(te))/t2_)*E_kurtosis
        #E_ballT2 = np.exp(-te/t2_)*E_ball
        return E_kurtosisT2
...
class G1BallT2T1inv(ModelProperties, IsotropicSignalModelProperties):
    r""" The Ball model [1]_ - an isotropic Tensor with one diffusivity
    with additional T2 decay and T1 inversion recovery
    ...
    def __call__(self, acquisition_scheme, **kwargs):
        ...
        S_ball = abs(1 - (2*np.exp(-ti/t1_)) + np.exp(-tr/t1_))*np.exp(-(te - np.min(te))/t2_)*E_ball
...
class G2ZeppelinT2(ModelProperties, AnisotropicSignalModelProperties):
    r""" The Zeppelin model [1]_ - an axially symmetric Tensor - typically used
    for extra-axonal diffusion with included t2 decay.
    ...
    .. [1] Panagiotaki et al.
           "Compartment models of the diffusion MR signal in brain white
            matter: a taxonomy and comparison". NeuroImage (2012)
    """
    _required_acquisition_parameters = ['bvalues', 'gradient_directions', 'TE']
    ...
    def __init__(self, mu=None, lambda_par=None, lambda_perp=None, t2=None):
        self.mu = mu
        self.lambda_par = lambda_par
        self.lambda_perp = lambda_perp
        self.t2 = t2
    ...
    def __call__(self, acquisition_scheme, **kwargs):
        ...
        #E_zeppelin = np.exp(-te/t2_) * _attenuation_zeppelin(
        #    bvals, lambda_par, lambda_perp, n, mu)
        E_zeppelin = np.exp(-(te - np.min(te))/t2_) * _attenuation_zeppelin(
            bvals, lambda_par, lambda_perp, n, mu)
