...
def main():
    ...
    acq_scheme, data, model, mask_rois = load_real_data(path_ROIs, path_diff, model_name)
    mask_glob = deepcopy(mask_rois)
    ...
    mask_rois = np.reshape(mask_rois, nx*ny)
    mask_glob = np.reshape(mask_glob, nx*ny)

    # LSQ fitting
    lsq_fit = model.fit(acq_scheme, data, mask=mask_glob)
    parameter_vector_lsq = lsq_fit.fitted_parameters

    nsteps = 2000
    burn_in = 1000

    # choose mask (global or ROI)
    mask = deepcopy(mask_rois)
    mask[mask != 4] = 0
    mask[mask == 4] = 1  # WM only

    # hierarchical Bayesian fitting
    proc_start = time.time()
    acceptance_rate, param_conv, parameter_vector_bayes, parameter_vector_init, likelihood_stored, w_stored, lsq_fit \
        = fit_bayes.fit(model, acq_scheme, data, lsq_fit, mask, nsteps, burn_in)
